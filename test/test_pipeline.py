...
""")
    ...
    def retriever(self, test_env_file: str) -> Retriever:
        ...
        """
        return Retriever(max_results=10, min_similarity_threshold=0.1)

    @pytest.fixture(scope="class", autouse=True)
    def setup_and_cleanup_databases(
        self, test_env_file: str
    ...
    def test_loader_can_process_pdf(self, loader: Loader, test_pdf_path: Path) -> None:
        ...
        chunk_count: int = loader.process_pdf_file(test_pdf_path)
    ...
    def test_loader_stores_data_in_mongodb(self, loader: Loader) -> None:
        ...
        total_docs: int = collection.count_documents({})
    ...
    def test_loader_stores_vectors_in_qdrant(self, loader: Loader) -> None:
        ...
        collection_info: CollectionInfo = loader.qdrant_client.get_collection(
            loader.qdrant_collection
        )
        ...
        points: List[Record] = loader.qdrant_client.scroll(
    ...
    def test_retriever_can_search_gnu_parallel_content(
        ...
        try:
            collection_info: CollectionInfo = retriever.qdrant_client.get_collection(
                retriever.qdrant_collection
            )
        ...
        results: List[RetrievalResult] = retriever.search(query)
        print(f"Results with threshold 0.0: {len(results)}")

        if len(results) == 0:
            # Try an even simpler search
            simple_results: List[RetrievalResult] = retriever.search("GNU")
            print(f"Simple 'GNU' search results: {len(simple_results)}")

            if len(simple_results) == 0:
                # Debug: List available collections
                collections: CollectionsResponse = (
                    retriever.qdrant_client.get_collections()
                )
                print(
                    f"Available Qdrant collections: {[c.name for c in collections.collections]}"
                )
    ...
    def test_retriever_search_for_specific_commands(self, retriever: Retriever) -> None:
        ...
        for query in test_queries:
            ...
            assert isinstance(results, list), (
                f"Results should be a list for query: {query}"
            )
    ...
    def test_retriever_similarity_threshold_filtering(
        self, retriever: Retriever
    ) -> None:
        ...
        results_low: List[RetrievalResult] = retriever.search(query)

        # Search with high threshold
        results_high: List[RetrievalResult] = retriever.search(query)
    ...
    def test_retriever_max_results_limiting(self, retriever: Retriever) -> None:
        ...
        results_small: List[RetrievalResult] = retriever.search(query)
        results_large: List[RetrievalResult] = retriever.search(query)
        ...
        if len(results_large) >= 3:
            ...
            assert len(results_small) == 3, (
                "Should return exactly 3 results when available"
            )
    ...
    def test_retriever_empty_and_invalid_queries(self, retriever: Retriever) -> None:
        ...
        # Empty query
        empty_results: List[RetrievalResult] = retriever.search("")
        assert isinstance(empty_results, list), "Should return list for empty query"

        # Very unlikely to match query
        unlikely_results: List[RetrievalResult] = retriever.search(
            "quantum mechanics biochemistry astrophysics"
        )
        assert isinstance(unlikely_results, list), (
            "Should return list for unlikely query"
        )
    ...
    def test_end_to_end_pipeline_integrity(
        ...
        mongo_count: int = loader.mongo_collection.count_documents({})
        qdrant_info: CollectionInfo = loader.qdrant_client.get_collection(
            loader.qdrant_collection
        )
        qdrant_count: int | None = qdrant_info.points_count
        ...
        results: List[RetrievalResult] = retriever.search("GNU")
        ...
        for result in results:
            ...
            assert mongo_doc is not None, (
                f"Chunk {result.chunk_id} not found in MongoDB"
            )
            assert mongo_doc["text"] == result.text, (
                "Text mismatch between retriever and MongoDB"
            )
            assert mongo_doc["source_file"] == result.source_file, (
                "Source file mismatch"
            )
            assert mongo_doc["chunk_index"] == result.chunk_index, (
