...
class TestSemanticQueryLayer:
    ...
    def ensure_test_data_exists(self, test_env_file: str):
        ...
        if chunk_count == 0:
            pytest.skip("No data found in production MongoDB. Start Docker Compose and load data first.")
        
        qdrant_client = QdrantClient(host="localhost", port=6333)
        try:
            collection_info = qdrant_client.get_collection("embeddings")
            if collection_info.points_count == 0:
                pytest.skip("No vector data found in production Qdrant. Start Docker Compose and load data first.")
        except Exception:
            pytest.skip("Qdrant collection not found. Start Docker Compose and load data first.")
        
        mongo_client.close()
        print(f"Production data available: {chunk_count} chunks, {collection_info.points_count} vectors")
    ...
    def test_retrieve_documents_tool_with_real_data(self, semql_with_real_retriever: SemanticQueryLayer):
    ...
    def test_retrieve_documents_tool_with_mock_data(self, semql_with_mock_retriever: SemanticQueryLayer):
    ...
    def test_query_response_structure(self, mock_agent_class, mock_client_class, semql_with_mock_retriever: SemanticQueryLayer):
    ...
    @patch('biblioteq.semql.ChatCompletionClient')
    def test_query_error_handling(self, mock_client_class, semql_with_mock_retriever: SemanticQueryLayer):
    ...
    def test_retrieval_tool_data_formatting(self, semql_with_real_retriever: SemanticQueryLayer):
        ...
        async def run_test():
            ...
            if result.total_results > 0:
                ...
                print(f"Data formatting test passed: {result.total_results} chunks formatted correctly")
